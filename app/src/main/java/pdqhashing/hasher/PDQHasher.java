...
public class PDQHasher {
      ...
      throws IOException
  {
    ...
    t1 = System.nanoTime();
    HashAndQuality rv = fromBitmap(img, buffer1, buffer2, buffer64x64, buffer16x64, buffer16x16);
    t2 = System.nanoTime();
  ...
  public HashAndQuality fromBitmap(
          Bitmap img,
    ...
    float[] luma) // image numRows x numCols as row-major array
  {
    ...
    for (int i = 0; i < numRows; i++) {
      for (int j = 0; j < numCols; j++) {
        int rgb = img.getPixel(j, i); // xxx check semantics of these packed-as-int pixels
        int r = (rgb >> 16) & 0xff;
        int g = (rgb >> 8) & 0xff;
        int b = rgb & 0xff;
        luma[i * numCols + j] =
      ...
      throws IOException
  {
    ...
    t1 = System.nanoTime();
    HashesAndQuality rv = dihedralFromBitmap(img, buffer1, buffer2, buffer64x64, buffer16x64,
  ...
  // This is coded with an eye to the future wherein we could hash video frames,
  // all of which would be the same dimension, and the buffers could be reused.
  public HashesAndQuality dihedralFromBitmap(
          Bitmap img,
    float[] buffer1, // image numRows x numCols as row-major array
    float[] buffer2, // image numRows x numCols as row-major array
    float[][] buffer64x64,
